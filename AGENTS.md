...
- `assets/svg/star-growth.svg` 由 `.github/workflows/refresh-star-growth.yml` 每天 09:23（北京时间）自动重绘，星数有变化才提交（github-actions[bot] 名义）。本地手动刷新仍用 `python3 scripts/gen-star-growth.py`（依赖已登录的 gh CLI）。
